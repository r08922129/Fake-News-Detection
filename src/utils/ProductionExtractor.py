...
class ProductionExtractor(AbstractFeatureExtractor):
    ...
    def __init__(self, config):
        ...
        if config.collectProductionFromCorpus:
            for filePath in config.pathesToParseTree:
                with open(filePath) as f:
                    trees = [json.loads(line) for line in f.readlines()]
                    for tree in trees:
                        self._countRewriteRules(tree)
                    ...
                    for sent in sent_tokenize(line):
                        try:
                            ...
                            for sentence in annotate.sentence:
                                ...
                                numberOfProduction += self._accumulateProduction(tree, array)
                        except:
                            continue
    ...
    def extractByTree(self, article_trees):
        out = []
    ...
    def _accumulateProduction(self, tree, array) -> int:
